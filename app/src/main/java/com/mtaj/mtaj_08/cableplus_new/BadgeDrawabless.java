...
public class BadgeDrawabless extends Drawable {
	...
	public BadgeDrawabless(Context paramContext) {
		this.mTextSize = paramContext.getResources().getDimension(
				R.dimen.textsize_10);
		this.mBadgePaint = new Paint();
		this.mBadgePaint.setColor(Color.parseColor("#ff0000"));
	...
	public void draw(Canvas paramCanvas) {
		...
		if (Integer.parseInt(this.mCount) < 10 && Integer.parseInt(this.mCount) == 0) {
			circleRadius = Math.min(width, height)/5.0f+ 9.5F;
		} else {
			circleRadius = Math.min(width, height)/5.0f+ 9.5F;
		}
		float circleX = width - circleRadius + 0.2F;
		float circleY = circleRadius - 0.5f;
